...
if (MATT) {
  ...
  crossvalfile <- paste(mattswd, '/data/cv_results_new.csv')
} else {
  ...
  crossvalfile <- paste(samswd, '/data/cv_results_new.csv')
}
...
calc_RMSE(.5*(boost1.preds + boost2.preds), validate$Outcome)
```

```{r}
kfcv_boost <- function(formula, data, num.trees, inter.depth, k) {
  ...
  for (i in 1:k) {
    fit <- data[folds != i,]
    leftout <- data[folds == i,]
    
    mod <- gbm(formula, data = fit, distribution = "gaussian",
               n.trees = num.trees, interaction.depth = inter.depth)
    preds <- predict(mod, newdata = leftout, n.trees = num.trees)
    results[i] <- calc_RMSE(preds, leftout$Outcome)
  }
...
kfcv_boost(curr.formula, train, num.trees, inter.depth = 5, k = 10)
